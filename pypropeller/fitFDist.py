import numpy as np
import scipy
from scipy.stats import t, f, chi2, rankdata
...
def fit_f_dist_robust(x, df1, covariate=None, winsor_tail_p=[0.05, 0.1]):  # DONE
    ...
    n = len(x)
    if n < 2: return res
    if n == 2: return (fit_f_dist(x, df1=df1))
    # check df1
    if len(df1) != n:
    ...
    if len(df1) > 1:
        ...
        i = (df1 < (df1_max - 1e-14))
        if i.any():
            if not covariate: s = non_robust['scale'] 
            else: s = non_robust['scale'][i]
            f1 = x[i]/s
            df2 = non_robust['df2']
...
def fit_f_dist(x, df1, covariate=None):  # DONE
    ...
    if m == 0:
        ...
        if np.any(x==0): print("Warning! Zero sample variances detected, have been offset away from zero.")
    x = pmax(x, 1e-5 * m)
    z = np.log(x)
